...
# config prep when re-launching replicas   
   ...
   for line in lines:
      if line.find("desired_temp") >= 0 and line.find("set") >= 0:
         ...
         print "new temperatures being set, re-launching#" + str(i) + "whose new temp=" + str(r)
   ifile.close()
...
##################################################################################  
  # Start BigJob
   ...
   # workingdirectory=os.getcwd() +"/agent"  # working directory for agent
    ...
    for i in range(0,NUMBER_BIGJOBS):
      ...
      bjs[i].start_pilot_job(lrms_url,
                            ...
                            queue,
                            project,
                            workingdirectory,userproxy,173)
   ...
   # jd.arguments = ["NPT.conf"]
    ...
    while 1: 
      if str(sjs[0].get_state())=="Running" or str(sjs[0].get_state())=="New" :
        break
      else:
        print "current state of subjob 0= " + str(sjs[0].get_state())
        time.sleep(10)
        pass 
    count=0
         ...
         # list=[]
          for f in range(0, NUMBER_REPLICAS):
            ...
            if((str(sjs[f].get_state())=="Done") and (f!=flag)):
                ...
                print "replica chosen for exchange is" + str(f) + " whose state: " +sjs[f].get_state()+ " whose temp: " + sjs[f].get_temp()
                print "replica for which selection was made" + str(i)+ " whose state: " +sjs[i].get_state()+ " whose temp: " + sjs[i].get_temp()
                print "assigning the new temepratures and re-starting the replicas"
                temp_a = str(sjs[f].get_temp())
                temp_b = str(sjs[i].get_temp())
                prepare_NAMD_config(temp_a, temp_b) 
                if i<RPB:
                  j=0
                  copy_with_saga(i)
                  sjs[i].submit_job(bjs[j].pilot_url, jds[i], str(i))
                  ...
                  copy_with_saga(i)
                  sjs[i].submit_job(bjs[j].pilot_url, jds[i], str(i))                
                prepare_NAMD_config(temp_b, temp_a)
                if f<RPB:
                  j=0
                  copy_with_saga(f)
